...
const getStepSize = (startTs, endTs) => {
  ...
  const matched =
    timeWindowsList.find((v) => v *200 > timeDiff) ||
...
const normaliseTime = (timestamp, step) => {
  ...
  return timestamp - ((timestamp - utcOffsetSeconds * 100) % (step * 100));
};
...
export class MimicMetrics {
  static fetchMetrics({ startTs, endTs }) {
    ...
    // Adjusting the getProportionateMinMax function to reflect the new ranges
